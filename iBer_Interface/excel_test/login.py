...
import json
import requests
from excel_test.readExcel import ExcelUtil
from excel_test.writeExcel import copy_excel, Write_excel
...
def send_requests(s, testdata):
    ...
    print("*******正在执行用例：-----  %s  ----**********" % str(test_nub))
    print("请求方式：%s, 请求url:%s" % (str(method), str(url)))
    ...
    if type == "data":
        ...
        body = bodydata
    if method == "post": print("post请求body类型为：%s ,body内容为：%s" % (str(type), str(body)))
...
if __name__ == "__main__":
    ...
    wirte_result(res, filename="testreport.xlsx")